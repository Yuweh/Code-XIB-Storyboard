...
class PinCodeVerificationController: SettingsRootViewController, UITextFieldDelegate{
    ...
    private let subscript_label: UILabel = UILabel()
        ...
        .set(value: "Please enter your 6-digit PIN to access Settings")
...
//        //        SettingsWireframe(navController).navigate(to: .AccessSettingsMainView)
//
//        let messages = NSAttributedString(string: "Thank you for using GBA Mobile Banking. Please don't forget your PIN Code to continually access Settings", attributes: [NSAttributedStringKey.font: UIFont(name: "Helvetica Neue", size: GBAText.Size.subContent.rawValue)!])
//
//        let content = NSMutableAttributedString()
//        content.append(messages)
//
//        SettingsWireframe(navController).presentSuccessPage(title: "PIN Code Update", message: content, doneAction: {
//            SettingsWireframe(navController).navigate(to: .AccessSettingsMainView)
...
//    }
    ...
    func existingPinUserValidator(enteredPinCode: String) {
        if enteredPinCode == pinUsers.pinNumber {
            //self.navigationItem.rightBarButtonItem?.isEnabled = true
            guard let navController = self.navigationController else { return }
            SettingsWireframe(navController).navigate(to: .AccessSettingsMainView)
        } else {
    ...
    func pinCodeValidator(pin: NSString) {
        let pinUserProfile = self.pinUsers
        let pinUserMobile = String(user.id)
        let enteredPin = String(pin)
        if pinUserProfile.pinNumber == nil {
            self.newPinUserCodeValidator(userNumber: pinUserMobile, enteredPinCode: enteredPin)
            //newPinUserCodeRepeater(enteredPinCode: enteredPin)
        } else {
            // For Existing PinUsers
            print("PinUser already set a PINcode")
    ...
    //EXPerimentor
    func newPinUserCodeValidator(userNumber: String, enteredPinCode: String) {
